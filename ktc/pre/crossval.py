...
def combine_train_test_val(path, kfolds=1):
    '''
    For a given modality combines the subjects of train val test folders and stores in a file + generates k folds CV data + stratified k folds CV + leave_one_out CV --from all subjects and stores separate files each with train and test subjects for each fold
    Args:
